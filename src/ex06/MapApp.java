...
public class MapApp {
    public static void main(String[] args) {
        ...
        // 게시물 조회 - 리스트, 페이지 정보
        // 키의 타입 - 문자열, 값의 타입
        // Map은 인터페이스라 직접 생성할 수 x -> 인터페이스의 구현체인 HashMap을 이용해서 생성
        // Map<키의 타입, 값의 타입>
        Map<String, Employee> map = new HashMap<>();

        // 키-값 쌍으로 데이터를 관리
        // 키는 중복될 수 없다 - 엎어써버림 !!
        map.put("사원1", new Employee("다현", 22, 0, 0, 0, null));
        map.put("사원2", new Employee("다현", 22, 0, 0, 0, null));
        map.put("사원3", new Employee("다현2", 22, 0, 0, 0, null));

        // map의 데이터를 꺼내올 때는 키 값을 매개변수로 사용 !
        // get : 키에 저장된 값을 반환
        System.out.println(map.get("사원1"));
        System.out.println(map.get("사원2"));
        System.out.println(map.get("사원3"));

        System.out.println(map.containsKey("사원1"));
        ...
        list.add(new Employee("다현", 20, 0, 0, 0, null));
        list.add(new Employee("다민", 25, 0, 0, 0, null));
        Employee emp = new Employee("다영", 23, 0, 0, 0, null);
        list.add(emp);
        
        map1.put("title", "map을 학습 중입니다.");
        map1.put("dept", new DeptDTO("D1", "인사부", "S1"));
        map1.put("list", list);
