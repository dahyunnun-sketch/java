...
public class PageDto {
    // 사용자가 요청한 페이지 정보
    // 기본값 -1 (사용자가 요청한 페이지가 없는 경우 무조건 1페이지를 보여준다)
    private int page =1;
    // 페이지당 보여줄 게시물의 수 (페이지의 게시물 건수는 10페이지로 초기화)
    // 기본값 - 10
    private int size = 10;
    // 게시물의 총 건수 - 데이터베이스의 조회 결과 테이블의 게시물 총 건수
    private int totalCnt;

    // 게시물의 시작번호, 끝번호 : 데이터베이스 조회 할 때 필요한 정보
    // DB의 종류, 버전에 따라 사용여부가 결정됨
    private int sNo;
    private int eNo;
    ...
    // boolean 타입의 기본값은 false이므로 초기화 할 필요는 없다
    ...
    public String toString() {
        // 페이지 블럭의 시작번호 부터 끝번호 까지 반복문을 이용해서 반환
        String str = "";
        if (isPrev) {
            str += "◀ ";
        }
    ...
    public PageDto(int page, int totalCnt){
        ...
        // 2페이지를 요청한 경우 1번째 블럭 (1 2 3 4 5)이 보여져야 함
        // 페이지의 끝번호를 구함
        ePageNo = (int)Math.ceil(page/(double)blockSize) * blockSize;
        sPageNo = ePageNo - (blockSize-1);

        // prev 버튼은 시작페이지번호가 1이 아닐 때만 보여줌
        // isPrev : boolean 타입
        // != 비교연산의 결과는 boolean타입이 반환됨
        isPrev = sPageNo!=1;

        // 진짜 끝페이지 = 올림(게시물의 건수/페이지당 게시물수)
        // 99/10 = 9.9 진짜 끝 페이지 번호 : 10페이지
        // next버튼은 진짜 끝 페이지 번호보다 페이지의 끝번호가 크면 보여줌
        int realEndPageNo = (int)Math.ceil(totalCnt/(double)size);
        System.out.println("진짜 끝 페이지 번호 : " + realEndPageNo);
        isNext = realEndPageNo > ePageNo;
        // 진짜 끝번호보다 끝번호가 큰 경우
    ...
    public static void main(String[] args) {
        ...
        // 게시물의 시작번호
        System.out.println((int)(Math.ceil(totalCnt/(size*1.0))*page) - (size-1)); //끝번호
